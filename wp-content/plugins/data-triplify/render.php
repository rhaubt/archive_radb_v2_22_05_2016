...
<?php
				...
				foreach ( $wpdb->get_col( "DESC " . $tabela, 0 ) as $coluna ){
					...
					if($registro == null) {
						...
						if($registro->uri == true){
							$checked = 'checked';
							$uriValue = $registro->uri_value;
						}
						else {
							$checked = "";
							$uriValue = "";
					...
					$contador."- <input type='checkbox' class='uri' id='uri".$contador."' ".$checked."/>";
					if($uriValue == null || $uriValue == '')
						echo "<input class='input_uri_value' value='". $uriValue ."' id='uriValue".$contador."' style='display:none' />";
					else
						echo "<input class='input_uri_value' value='". $uriValue ."' id='uriValue".$contador."'/>";
					echo $coluna." => ".
			...
			jQuery(document).ready(function($) {
				...
				$("#id").click(function(){
					...
					$('.input_triplify').each(function(k,v,w){
						...
						if($.trim(v) != 'correspondence' &&  $.trim(v) != ''){
							...
							if(checkbox.is(":checked")){
								...
								var uriValueVariable = "#uriValue";
								var concatenateX = uriValueVariable.concat(contadorX);
								post_triplify.uriValue = $(concatenateX).val();
							} else {
								post_triplify.uri = 'false';
								post_triplify.uriValue = '';
							}
					...
					$.post(ajaxurl, data, function(response) { 
						<!-- Ver o que fazer em caso de falha-->
					});
				...
				$(".uri").on('click', function() {
					var $box = $(this);
					
					var id = $(this).attr('id');
					var contador = id.replace("uri", "");
					var contadorX = "#uriValue";
					
					if ($box.is(":checked")) {
						var elementoMostrar = contadorX.concat(contador);
						$(elementoMostrar).show();
					} else {
						var elementoEsconder = contadorX.concat(contador);
						$(elementoEsconder).val('');
						$(elementoEsconder).hide();
					}
				});
