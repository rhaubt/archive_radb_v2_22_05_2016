                ...
                if ($init != 0 && $init != 1) {
					...
					foreach($this->types as $type){
						...
						if($jaExisteNoBanco != null){
							$wpdb->update($tabela, array('tipo' => $type, 'coluna' => $keys[0], 'uri' => $object->uri, 'uri_value' => $object->uriValue, 'valor_correspondente' => $object->fullProperty), array('tipo' => $type, 'coluna' => $keys[0]));
						} else{
							$wpdb->insert($tabela, array('tipo' => $type, 'coluna' => $keys[0], 'uri' => $object->uri, 'uri_value' => $object->uriValue, 'valor_correspondente' => $object->fullProperty));
						}
					}
				}
