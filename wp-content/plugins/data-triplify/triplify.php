...
add_action('init', 'dt_rewrite_tag', 10, 0);
function dt_rewrite_tag() {
...
add_action('generate_rewrite_rules', 'triplify_add_rewrite_rules');
function triplify_add_rewrite_rules(){
...
add_action('template_redirect', 'triplify_template_redirect' );
function triplify_template_redirect(){
	global $wp_query;
...
function triplify_action_callback() {
	...
	foreach(array_values($_POST['arrayCorrespondencias']) as $opcoes){
		...
		if($valor_anterior_banco != null){
			$wpdb->update($tabela, array('tipo' => $post, 'coluna' => $coluna, 'uri' => $uri, 'uri_value' => $uriValue, 'valor_correspondente' => $opcoes["valor"]), array('tipo' => $post, 'coluna' => $coluna));
		} else {
			$wpdb->insert($tabela, array('tipo' => $post, 'coluna' => $coluna, 'uri' => $uri, 'uri_value' => $uriValue, 'valor_correspondente' => $opcoes["valor"]));
		}
