...
class dt_TYPE_TEXT {
	...
	function dt_show_data($type, $structure){
		...
		$posts_array = array( 'posts_per_page' => 100000000, 'post_type' => $type );
		$posts = get_posts($posts_array);
		
		//$posts = $wpdb->get_results("SELECT * FROM wp_posts, wp_postmeta WHERE post_type = " + $type + " AND ID = post_id", OBJECT);
			...
			foreach($array as $valores){
				//if (array_key_exists($valores[0], $post)) { 
					...
					if(strpos($valores[1], ":")){// if prefix contains ':'
						...
						$uri = $wpdb->get_row("SELECT uri, uri_value FROM {$wpdb->prefix}triplify_configurations WHERE tipo='".$type."' and coluna='".$valores[0]."'", OBJECT);//see if this is a URI column or not
						$object = new dt_prefixColumnUri();
						$object->prefix = $explode[0];
						$object->coluna = $explode[1];
						$object->uri = $uri->uri;
						$object->uriValue = $uri->uri_value;
