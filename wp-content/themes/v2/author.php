...
<div id="primary" class="site-content">
	<div id="content" role="main">
			...
			<div style="width: 600px; height: 100vh; float: right; overflow-y: scroll; overflow-x:hidden; box-shadow: 0px 0px 50px rgb(0, 0, 0);">
...
<!--     Online Presence  -->
						...
						<span title="find us on social media eg website, twitter, academia, facebook etc." class="dashicons dashicons-share" style="position: relative; top: 0px; left: 0px; z-index: 5;"></span>
							...
							<a style="text-decoration:none;" title="Official Website" style="text-decoration:none;" target="_blank" href="<?php echo $curauth->user_url; ?>"><small><span class="dashicons dashicons-share-alt"></span></small></a>
							<?php endif; ?>
							<a style="text-decoration:none;" target="_blank" href="<?php echo get_author_feed_link($curauth->ID); ?>"><span class="dashicons dashicons-rss"></a></br></br>					
  			...
  			<div style="background-color:#eeeeee; padding:2px;">
						<div>	
								...
								foreach($comments as $comment) :
									echo('<p style="background-color:#e0e0e0; padding: 7px; text-align:left; font-size:12px;"> Commented on <a href='. get_permalink($comment->comment_post_ID).'>'.get_field( "RAE_0003", $comment->comment_post_ID).' ('. get_the_title($comment->comment_post_ID) . ')</a></p>' . '<div style="border-bottom: solid 1px black; padding: 2px 2px 20px 2px;">' . get_avatar( get_the_author_meta( 'ID' ), 44 ) . '<p style="font-size:14px; margin-left:40px; margin-top: 12px;">' . $comment->comment_author . /* . the_author_posts_link($comment->get_comment_ID) .*/'</br>' . '<span style="font-size:12px;">' . $comment->comment_date . '</span>' . '</p><br />' . $comment->comment_content . '</div>');
								endforeach;
								?>
						</div>
			</div>							
